...
const EditSupplier = () => {
  ...
  useEffect(() => {
    ...
    const fetchSupplier = async () => {
      try {
        const response = await axios.get(`${base_url}supplier/getsupplierbyid/${supplierId}`);
  ...
  const handleUpdateSupplier = async (event) => {
    ...
    try {
      const response = await axios.put(`${base_url}supplier/update-supplier`, updatedData, config);
  ...
  const handleDeleteProduct = async (productId) => {
    try {
      const response = await axios.put(`${base_url}supplier/delete-supplier-products`, {
        productId: productId,
  ...
  return (
    <div className="px-5 py-2">
      ...
      <form onSubmit={handleUpdateSupplier} className="d-flex flex-column">
        <label>Name:</label>
        <Input
          value={supplierDetails.supplier_name}
          onChange={(e) =>
            setSupplierDetails({ ...supplierDetails, supplier_name: e.target.value })
          }
        />
        <label>Email:</label>
        <Input
          ...
          onChange={(e) =>
            setSupplierDetails({ ...supplierDetails, supplier_email: e.target.value })
          }
        />
        <label>Phone:</label>
        <Input
          ...
          onChange={(e) =>
            setSupplierDetails({ ...supplierDetails, supplier_phone: e.target.value })
          }
        />
        <label>Address:</label>
        <Input
          ...
          onChange={(e) =>
            setSupplierDetails({ ...supplierDetails, supplier_address: e.target.value })
          }
        />
        ...
        <div>
          ...
          {supplier.length > 0 ? (
            <Row gutter={[16, 16]}>
              {supplier.map((product, index) => (
                <Col key={index} span={6}>
                  <Card
                    cover={<img alt="product" src={product.image_link} height={200} />}
                    actions={[
                      <Button
                        key={product.p_id}
                        type="danger"
                  ...
                  >
                    <Card.Meta
                      title={<Typography.Title level={5}>{product.p_id}</Typography.Title>}
                      description={<Typography.Paragraph>{product.p_title}</Typography.Paragraph>}
                    />
                  </Card>
                </Col>
              ))}
            ...
            <Typography.Paragraph>No products supplied by this supplier.</Typography.Paragraph>
          )}
        </div>
